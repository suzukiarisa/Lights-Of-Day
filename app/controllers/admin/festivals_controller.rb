class Admin::FestivalsController < ApplicationController
  ...
  def edit
    if admin_signed_in?
    @festival = Festoval.find(params[:id])
  else
    redirect_to root_path
  end
  end
  ...
  def autocomplete_festival_name
    festival_name_suggestions = Festival_name.autocomplete(params[:term]).pluck(:name)
  ...
  def create
    @festival_name = Festival_name.find_by(name: params[:festival][:festival_name_id])
